...
test("sanitizes workflow status against invalid or injected values",()=>{const r=assessIncident({fields:{earlyStatus:"<script>alert(1)</script>",notificationStatus:"invalid_status"}},new Date());assert.equal(r.timelines[0].workflowStatus,"draft");assert.equal(r.timelines[1].workflowStatus,"draft")});
